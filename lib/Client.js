...
const PING_REQUEST_TIMEOUT = 30000;
const V2VNC_LT_SERVER = 'vnctest.v2cloud.com';
...
class Client extends EventEmitter {
    constructor(options) {
        ...
        agent.on('online', () => {
            ...
            // start periodic ping
            this.pingTimeout = setTimeout(this.ping.bind(this), PING_TIMEOUT).unref()
        });
    ...
    async ping() {
        await new Promise((resolve, reject) => {
            const options = {
                hostname: `${this.id}.${V2VNC_LT_SERVER}`,
                path: '/ping',
                method: 'GET',
                agent: this.agent,
            ...
            const req = http.request(options, res => {
                if(res.statusCode === 200) {
                    this.debug('ping client status %s', res.statusCode);
                    res.on('data', d => {
                        const resData = d.toString();
                        this.debug(`client response ${resData}`);
                        
                        if(resData === this.id)
                            this.pingFailed = 0;
                        else {
                            // the tunnel from the same client Address but diffirent clientId
                            this.debug('not matching clientId, so destroy this client');
                            this.close();
                        }
                        resolve();
                    })
            ...
            req.on('socket', (socket) => {
                socket.setTimeout(PING_REQUEST_TIMEOUT, () => {
                    this.debug('PING REQUEST TIMEOUT destroy socket');
                    socket.destroy();
                    this.pingFailed++;
                    resolve();
                });
            ...
            req.on('error', error => {
                this.debug('ping client error %s', error);
                this.pingFailed++;
                resolve();
            })
        ...
        this.debug(`ping failed ${this.pingFailed}`, (new Date()).toUTCString());

        if(this.agent.closed){
            this.debug('Agent closed. Stop ping!');
            return;
        }

        if(this.pingFailed > 0)
            this.agent.availableSockets.forEach(socket => socket.destroy())
