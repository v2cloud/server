...
class Client extends EventEmitter {
    constructor(options) {
        ...
        agent.on('online', () => {
            ...
            // start periodic ping
            this.pingTimeout = setTimeout(this.ping, 30000).unref()
        });
    ...
    async ping() {
        ...
        this.debug(`ping failed ${this.pingFailed}`);
        if(this.pingFailed > 2)
            this.close()
        else
            this.pingTimeout = setTimeout(this.ping, 30000).unref()
    }
    ...
    pingClient() {
        return new Promise((resolve, reject) => {
            const options = {
                hostname: `${this.id}.vnctest.v2cloud.com`,
                port: 443,
                path: '/ping',
                method: 'GET',
                agent: this.agent
