...
class TunnelAgent extends Agent {
    constructor(options = {}) {
        ...
        // count for no more socket event
        this.noMoreSocketCount = 0;
    ...
    stats() {
        return {
            ...
            noMoreSocketEvents: this.noMoreSocketCount,
    ...
    _onConnection(socket) {
        // allow only the initial client address
        if(this.clientAddress && this.clientAddress !== socket.remoteAddress){
            this.debug('client address not matching!');
            socket.destroy();
            return false;
        }
        ...
        if (this.connectedSockets >= this.maxTcpSockets) {
            this.noMoreSocketCount++;
            if(this.noMoreSocketCount <= DEFAULT_MAX_SOCKETS) {
                this.debug('no more sockets allowed');
                socket.destroy();
                return false;
            }
            // not match number of connections between lt-server and lt-client (lt-server may hold broken connections)
            this.debug('lt-client is spamming lt-server due to mismatched number of healthy connections, close all connections to establish new set');
            this.availableSockets.forEach(soc => soc.destroy());
        }
        this.noMoreSocketCount = 0; // reset
        ...
        if (this.connectedSockets === 0) {
            ...
            if(!this.clientAddress) { // initial client address
                this.clientAddress = socket.remoteAddress;
                this.debug(`Set client address to ${this.clientAddress}`);
            }
        }

        this.connectedSockets += 1;
        this.debug('new connection from: %s:%s remote client %s:%s [#tunnel %s]', socket.address().address, 
            socket.address().port, socket.remoteAddress, socket.remotePort, this.connectedSockets);
